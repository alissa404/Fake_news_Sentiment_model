...
_, words2array = joblib.load(
    '/home/alissa77/WWW2021 copy/resources/Chinese/大连理工大学情感词汇本体库/preprocess/words2array_27351.pkl')
print('[Dalianligong]\tThere are {} words, the dimension is {}'.format(len(words2array), words2array['快乐'].shape))
...
def symbols_count(text):
    ...
    ellip = [] 
    for i in text:
        excl.append((i.count('！') + i.count('!')) / len(i))
        ques.append((i.count('？') + i.count('?')) / len(i))
        comma.append((i.count(',') + i.count('，')) / len(i))
        dot.append((i.count('.') + i.count('。')) / len(i))
        ellip.append((i.count('..') + i.count('。。')) / len(i))
    excl = sum(excl)
    ques = sum(ques)
    comma = sum(comma)
    dot = sum(dot)
    ellip = sum(ellip)
    #print(excl, ques, comma, dot, ellip)
